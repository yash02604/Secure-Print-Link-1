...
export const PrintJobProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        setError('Could not connect to server. Check backend and Appwrite configuration.');
        toast.error('Could not connect to backend. Print jobs are Appwrite-only.');
      } finally {
        setIsFetching(false);
      }
    };
  ...
  const submitPrintJob = async (jobData) => {
    ...
    try {
      ...
      const message = error?.response?.data?.error || error.message || 'Failed to submit print job';
      setError(message);
