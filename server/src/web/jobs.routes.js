...
// Create a share link for cross-browser viewing (owner-controlled)
router.post('/:id/share', (req, res) => {
  const db = req.db;
  const { id } = req.params;
  const { token, expiresInMinutes, maxViews } = req.body || {};
  ...
  try {
    ...
    const minutes = Math.max(1, parseInt(expiresInMinutes || 60));
    const views = Math.max(1, parseInt(maxViews || 3));
    const expiresAt = new Date(Date.now() + minutes * 60 * 1000).toISOString();
    const shareId = nanoid();
    const shareToken = nanoid(32);

    db.prepare(`
      INSERT INTO share_links (id, jobId, token, viewsRemaining, expiresAt, createdAt)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(shareId, id, shareToken, views, expiresAt, new Date().toISOString());

    const host = req.headers['x-forwarded-host'] || req.headers.host;
    const protocol = req.headers['x-forwarded-proto'] || (req.secure ? 'https' : 'http');
    const computedOrigin = `${protocol}://${host}`;
    const origin = process.env.PUBLIC_BASE_URL || req.headers.origin || computedOrigin;
    const shareLink = `${origin.replace(/\/$/, '')}/release/${id}?share=${shareToken}`;

    res.json({ success: true, shareLink, token: shareToken, expiresAt, maxViews: views });
  } catch (err) {
...
// View via share token (does not flip isViewed)
router.get('/share/:shareToken', (req, res) => {
  const db = req.db;
  const { shareToken } = req.params;

  try {
    const link = db.prepare('SELECT * FROM share_links WHERE token = ?').get(shareToken);
    if (!link) return res.status(404).json({ error: 'Share link not found' });

    const now = Date.now();
    if (now >= new Date(link.expiresAt).getTime()) {
      db.prepare('DELETE FROM share_links WHERE id = ?').run(link.id);
      return res.status(410).json({ error: 'Share link has expired' });
    }
    if (link.viewsRemaining <= 0) {
      db.prepare('DELETE FROM share_links WHERE id = ?').run(link.id);
      return res.status(410).json({ error: 'Share link has no views remaining' });
    }

    const job = db.prepare('SELECT * FROM jobs WHERE id = ?').get(link.jobId);
    if (!job) return res.status(404).json({ error: 'Job not found' });

    const document = db.prepare('SELECT * FROM documents WHERE jobId = ?').get(link.jobId);
    if (!document) return res.status(404).json({ error: 'Document not found' });

    const decryptedBuffer = decryptDocumentForJob(document.content, link.jobId);
    const base64 = decryptedBuffer.toString('base64');
    const documentData = {
      dataUrl: `data:${document.mimeType};base64,${base64}`,
      mimeType: document.mimeType,
    ...
    db.prepare('UPDATE share_links SET viewsRemaining = viewsRemaining - 1 WHERE id = ?').run(link.id);

    res.json({
      success: true,
      jobId: link.jobId,
      document: documentData,
      viewsRemaining: link.viewsRemaining - 1,
